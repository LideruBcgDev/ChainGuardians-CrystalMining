...
class CrystalMining():
    ...
    def __init__(self):
        ...
        with open('./abi/SKALE.CrystalMining.json') as f:
            self.CrystalMiningAbi = json.load(f)

        self.miningAddr = Web3.toChecksumAddress(src.config.TokenAddress.SKALE_miningAddr)
    ...
    def loopMine(self, second, times):
        ...
        while count < times:
            ...
            if time_difference < second:
                time.sleep(second - time_difference)
        ...
        for i in range(cnt):
            ...
            try:
                ...
                tx_content = {
                    ...
                    "from": walletAddr,
                    'gas': 300000,
                    "gasPrice": Web3.toWei(0.0001, "gwei"),
                }

                #airdrop関数
                tx = contract.functions.mineCrystalFree().buildTransaction(tx_content)
                ...
                print(f'{self.LogTime()} {address_index}:{walletAddr}:{result}')
